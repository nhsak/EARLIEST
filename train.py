...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, drop_last=True)
    ...
    for epoch in range(args.nepochs):
        ...
        for i, (X, y) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.nepochs}")):
            ...
            training_locations.append(halting_points[:,0].cpu().detach().numpy())
    ...
    for i, (X, y) in enumerate(tqdm(validation_loader, desc="Validation")):
        ...
        validation_locations.append(halting_points[:,0].cpu().detach().numpy())
    ...
    # --- save model ---
    torch.save(model.state_dict(), model_save_path+"image_model.pt")
